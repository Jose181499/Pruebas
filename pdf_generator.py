# pdf_generator.py - VERSIÓN COMPLETA CON GUÍAS DE REMISIÓN
...
"""
    ...
    # GENERAR COTIZACIÓN
    # ============================================================
    # pdf_generator.py - Reemplazar el método _generar_cotizacion
...
def _generar_cotizacion(self, datos_cotizacion):
    ...
    try:
        ...
        # Obtener el template en memoria
        template_content = self._obtener_template_cotizacion()
        
        # Renderizar usando Jinja2
        from flask import render_template_string
        
        html_content = render_template_string(template_content, **datos_cotizacion)
        
        fecha = datetime.now().strftime('%Y%m%d_%H%M%S')
        pdf_file = f"cotizacion_{datos_cotizacion.get('numero_cotizacion', 'sin_numero')}_{fecha}.pdf"
